...
uses
  Windows, Messages, SysUtils, Classes, Graphics, SFXInterface, ZipMsg, DZUtils, SFXStructs;
...
begin
  ...
  if hFindSFX <> 0 then
  begin
    hFindSize := FindResource(HInstance, PChar(DzSfxID+SLanguage+BinSizeID), RT_RCDATA);
    if (hFindSize <> 0) and (SizeOfResource(HInstance, hFindSize) >= sizeof(Cardinal)) then
    begin
      hRes := LoadResource(hInstance, hFindSize);
      if hRes <> 0 then
      begin
        ...
        begin
          ...
          if (cSize > 0) and (SizeOfResource(HInstance, hFindSFX) >= cSize) then
          begin
            hRes := LoadResource(HInstance, hFindSFX);
            if hRes <> 0 then
            begin
              pData := LockResource(hRes);
              if pData <> nil then
...
//      raise EDelphiZip.CreateResDisp(SF_StringToLong, True);
    ...
    begin
      ...
      p := PChar(Memory) + sizeof(TSFXFileHeader);
      if CaptionSize > 0 then
      begin
        StrPCopy(p, FDialogTitle);
        p := p + CaptionSize;
      end;
      if PathSize > 0 then
      begin
        StrPCopy(p, FDefaultExtractPath);
        p := p + PathSize;
      end;
      if CmdLineSize > 0 then
      begin
        StrPCopy(p, FCommandLine);
        p := p + CmdLineSize;
      end;
      if RegFailPathSize > 0 then
      begin
        StrPCopy(p, FFallbackExtractPath);
        p := p + RegFailPathSize;
      end;
      if StartMsgSize > 0
      then
      ...
      with PSFXFileEndOfHeader(PChar(Memory)+iSize)^ do
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Move((PChar(@FEOC)+FPosition)^, Buffer, Result);
  ...
  begin
    ...
    begin
      ...
      then
        ...
        sr1 := PChar(sr1)+'='+AnsiUpperCase(sr);
      if sr1 <> '' then
      begin
