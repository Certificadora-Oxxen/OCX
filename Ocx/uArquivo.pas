...
uses Sysutils, Variants, uLibZipM;
...
begin
  try
    ...
    FInicializado := True;
    sTMP := ValorColuna[1];
    FDtaGeracao := StrToDateTime(sTMP);
...
begin
  if not(FInicializado) then Abort;
  if (Coluna < 1) or (Coluna > FNumeroColunas) then Exit;
