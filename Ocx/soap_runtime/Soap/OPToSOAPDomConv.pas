...
{ NOTE: Enabling FIX_ELEM_NODE_NS introduces a new member, hence breaks interface
        compatibiilty with previously released copy of this file. You may define
        FIX_ELEM_NODE_NS if you do not need interface compability }
{$IFDEF HIGHLANDER_UP}
  {$DEFINE FIX_ELEM_NODE_NS}
{$ENDIF}

{$DEFINE FASTER_CONVERTER}
{$DEFINE CHECK_FOR_NS_SWITCH}

{$IF FALSE}
  {$DEFINE NO_NAMESPACE_ON_NIL_NODE}
{$IFEND}
...
begin
  ...
  { NOTE: Typically you don't want us to UTF8 Encode if you've requested
    ...
    // Envio
//  XMLDoc.SaveToFile('C:\Jerry\TQI\Herdom\TestesJerry\envio.xml');
...
//  XMLDoc.SaveToFile('C:\Jerry\TQI\Herdom\TestesJerry\retorno.xml');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (soDocument in Options) then
{$IFDEF FIX_ELEM_NODE_NS}
          ArNode := Node.AddChild(NodeName, FElemNodeNamespace)
{$ELSE}
...
begin
  Result := not IsStoredPropConst(nil, PropInfo)
            and (Kind <> tkDynArray)
            and (Kind <> tkVariant)
            and ((Kind <> tkClass) or
...
{$ELSE}
        begin
          if (FElemNodeNamespace <> '') then
            InstNode := ParentNode.AddChild(NodeName, FElemNodeNamespace)
          else
            InstNode := ParentNode.AddChild(NodeName, TypeNamespace);
        end;
{$ENDIF}
  ...
  begin
    ...
    begin
{$IFDEF FIX_ELEM_NODE_NS}
      PrevElemNamespace := FElemNodeNamespace;
      FElemNodeNamespace := TypeNamespace;
{$ENDIF}
      GetMem(PropList, Count * SizeOf(Pointer));
      try
...
{$ELSE}
                  begin
                    if (soDocument in Options) then
                      CreateNilNode(InstNode, ExtPropName, FElemNodeNamespace)
                    else
                      CreateNULLNode(RootNode, InstNode, ExtPropName);
                  end;
{$ENDIF}
...
{$ENDIF}
            ...
            begin
              ...
              ConvertNativeArrayToSoap(RootNode, InstNode, ExtPropName,
                                       ...
                                       (xoInlineArrays in SerialOpts) or (not IsStoredProp(Instance, PropList[I])));
            { Variant property }
...
{$ENDIF}
        ...
        FreeMem(PropList, Count * SizeOf(Pointer));
{$IFDEF FIX_ELEM_NODE_NS}
        FElemNodeNamespace := PrevElemNamespace;
{$ENDIF}
      end;
    end;
  end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                { NOTE: Better check if the Instance already has the object property set?? }
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  InliningArrays := (xoInlineArrays in SerialOpts) or (not IsStoredProp(Instance, PropList[I]));
...
begin
  ...
  { TODO : There are instances where we don't want the empty namespace on the nil node. Investigate. }
{$IFDEF NO_NAMESPACE_ON_NIL_NODE}
...
begin
  ...
  begin
    ...
    if (soDocument in Options) then
{$IFNDEF CHECK_FOR_NS_SWITCH}
      Result := ParentNode.AddChild(NodeName)
{$ELSE}
    begin
      if (URI <> '') and (FindPrefixForURI(RootNode, ParentNode, URI, False) = '') then
        Result := ParentNode.AddChild(NodeName, URI)
      else
        { NOTE: For form="unqualified" what we want here is:
             AddChild(NodeName, '', False); !!            }
        Result := ParentNode.AddChild(NodeName);
    end
{$ENDIF}
    else
...
{$IFDEF FIX_ELEM_NODE_NS}
  ...
  begin
    if (FElemNodeNamespace <> '') then
      Result := CreateTypedNode(RootNode, ParentNode, NodeName, FElemNodeNamespace, TypeName)
    else
